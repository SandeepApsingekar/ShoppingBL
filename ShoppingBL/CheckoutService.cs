...
namespace ShoppingBL
{
    public class CheckoutService:ICheckout
    {
        ...
        public decimal GetTotalCostWithOffer(List<Product> products)
        {
            decimal totalCost = 0.0m;
            int totalApples = 0;
            decimal applePrice = 0.0m;
            ...
            if (products.Any())
            {
                ...
                var appleReminder = 0;
                var totalApplesToCalculate = Math.DivRem(totalApples, 2, out appleReminder);
                var orangeReminder = 0;
                var totalOrangesToCalculate = Math.DivRem(totalOranges, 3, out orangeReminder) * 2;

                switch (appleReminder)
                {
                    case 0:
                        totalCost += totalApplesToCalculate * applePrice;
                        break;
                    default:
                        totalCost += totalApplesToCalculate * applePrice + applePrice;
                        break;
                }

                switch (orangeReminder)
                {
                    case 0:
                        totalCost += (totalOrangesToCalculate * orangePrice);
                        break;
                    default:
                        totalCost += totalOrangesToCalculate * orangePrice + orangeReminder * orangePrice;
                        break;
                }
            }
